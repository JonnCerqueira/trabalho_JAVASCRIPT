...
function emailJaCadastrado(email, cpfIgnorado = null) {
  return clientes.some(c => c.email === email && c.cpf !== cpfIgnorado);
}

// -------------------- TODO 7: resetar formulário --------------------
...
function validarCliente(cliente, modoEdicao = false) {
  ...
  if (!cliente.uf) {
    erroUF.textContent = "UF obrigatório.";
...
form.addEventListener("submit", e => {
  ...
  if (cpfEmEdicao === null) {
    ...
    // TODO: edição
    cliente.cpf = cpfEmEdicao; // CPF fixo
    if (!validarCliente(cliente, true)) {
      mostrarMensagemGlobal("Corrija os erros antes de salvar.", "erro");
      return;
    }
    const idx = clientes.findIndex(c => c.cpf === cpfEmEdicao);
    if (idx !== -1) {
...
listaClientes.addEventListener("click", e => {
  ...
  if (e.target.classList.contains("btn-edit")) {
    // TODO: carregar cliente para edição
